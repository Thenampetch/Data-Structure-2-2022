...
public class BSTRecursive {
	...
	public int numNodesLessThan(BSTNode n,int v) {
		...
		//check if less than threshold value count itself and check both left and right.
	    if (n.data < v) {
	        return 1 + numNodesLessThan(n.left, v) + numNodesLessThan(n.right, v);
	    } 
	    //check if more than threshold value then continue to check on the left.
	    else {
	...
	public int numLeavesLessThan(BSTNode n, int v) {
		...
		//check less than threshold.
			...
			//check it is a leave or not
			if (n.left == null && n.right == null) {
				return 1;
			} else {
				//if not a leave continue check next node both left and right.
	...
	public int nextOf(BSTNode n,int v) {
		if (n == null) {
			...
			// n.value > v, search in the left subtree
	        int left = nextOf(n.left, v);
	        if (left == -1 || left <= v) {
	            // left subtree does not have a next value or its next value is less than or equal to v
	            return n.data;
	        } else {
	            // left subtree has a next value greater than v
	            return left;
	        }
		}
